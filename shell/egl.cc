...
Egl::Egl(wl_display* native_display, bool debug_egl, int buffer_size)
    : m_native_display(native_display),
      m_buffer_size(buffer_size),
      m_pfDebugMessageControl(nullptr),
      m_pfQueryDebug(nullptr),
      m_pfLabelObject(nullptr) {
  FML_DLOG(INFO) << "+ Egl()";

  m_dpy = get_egl_display(EGL_PLATFORM_WAYLAND_KHR, m_native_display, nullptr);
  assert(m_dpy);
  ...
  for (size_t i = 0; i < kEngineInstanceCount; i++) {
    m_context[i] = eglCreateContext(m_dpy, m_config, EGL_NO_CONTEXT,
                                    kEglContextAttribs.data());
    assert(m_context[i]);
    FML_DLOG(INFO) << "m_context[" << i << "] = " << m_context[i];

    m_resource_context[i] = eglCreateContext(m_dpy, m_config, m_context[i],
                                             kEglContextAttribs.data());
    assert(m_resource_context[i]);
    FML_DLOG(INFO) << "m_resource_context[" << i
                   << "] = " << m_resource_context[i];
  }

  m_texture_context = eglCreateContext(m_dpy, m_config, m_context[0],
                                       kEglContextAttribs.data());
...
EGLSurface Egl::create_egl_surface(Egl* egl,
                                   void* native_window,
                                   const EGLint* attrib_list) {
  ...
  if (create_platform_window)
    return create_platform_window(egl->m_dpy, egl->m_config, native_window,
                                  attrib_list);

  return eglCreateWindowSurface(egl->m_dpy, egl->m_config,
...
[[maybe_unused]] void Egl::ReportGlesAttributes(EGLConfig* configs,
                                                EGLint count) {
...
bool Egl::MakeCurrent(size_t index) {
  ...
  if (egl_error != EGL_SUCCESS) {
    FML_LOG(ERROR) << "(" << index << ") Make current failed: " << egl_error;
    assert(false);
  }
  return true;
}
...
bool Egl::SwapBuffers(size_t index) {
  // FML_DLOG(INFO) << "(" << index << ") Egl::SwapBuffers";
  eglSwapBuffers(m_dpy, m_egl_surface[index]);
  EGLint egl_error = eglGetError();
  if (egl_error != EGL_SUCCESS) {
...
bool Egl::MakeResourceCurrent(size_t index) {
  // FML_DLOG(INFO) << "(" << index << ") Egl::MakeResourceCurrent";
  eglMakeCurrent(m_dpy, EGL_NO_SURFACE, EGL_NO_SURFACE,
                 m_resource_context[index]);
  EGLint egl_error = eglGetError();
  if (egl_error != EGL_SUCCESS) {
    FML_LOG(ERROR) << "(" << index
                   << ") MakeResourceCurrent failed: " << egl_error;
    assert(false);
  }
  return true;
}
